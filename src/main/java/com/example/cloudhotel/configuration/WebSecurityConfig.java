...
import com.example.cloudhotel.security.jwt.AuthEntryPointJwt;
import com.example.cloudhotel.security.jwt.AuthTokenFilter;
...
//  스프링 시큐리티 룰을 무시하게 하는 Url 규칙(여기 등록하면 규칙 적용하지 않음)
  ...
  public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
    http.cors()
        ...
        .authorizeRequests().antMatchers("/api/auth/**","/api/room/**","/api/combo/**",
                    "/api/etc/**","/api/like/**","/api/user/**","/api/nonmember/**","/api/notice/**","/api/menu/**",
                    "/api/facilities/**", "/api/dining/**","/api/reservation/**","/api/booking/**","/api/reserve/**"
            ,"/api/rmypage/**").permitAll() // 이 url 은 모든 사용자 접근 허용
        .antMatchers("/api/test/**").permitAll() // 이 url 은 모든 사용자 접근 허용
