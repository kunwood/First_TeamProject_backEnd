...
@Table(name="TB_COMBO")
@SequenceGenerator(
        name = "SQ_COMBO_GENERATOR"
        , sequenceName = "SQ_COMBO"
        , initialValue = 1
        , allocationSize = 1
)
@Getter
...
@SQLDelete(sql = "UPDATE TB_COMBO SET DELETE_YN = 'Y', DELETE_TIME=TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS') WHERE PNO = ?")

public class Combo extends BaseTimeEntity{
    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE,
    generator = "SQ_COMBO_GENERATOR")
    private Integer pno;
