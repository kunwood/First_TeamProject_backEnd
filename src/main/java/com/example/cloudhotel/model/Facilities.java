 ...
 */
@Entity
@Table(name="TB_FACILITIES")
@SequenceGenerator(
        name = "SQ_FACILITIES_GENERATOR"
        , sequenceName = "SQ_FACILITIES"
        , initialValue = 1
        , allocationSize = 1
)
@Getter
...
@SQLDelete(sql = "UPDATE TB_FACILITIES SET DELETE_YN = 'Y', DELETE_TIME=TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS') WHERE FNO = ?")

public class Facilities extends BaseTimeEntity{
    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE,
    generator = "SQ_FACILITIES_GENERATOR")
    private Integer fno;
