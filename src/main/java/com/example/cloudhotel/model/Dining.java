 ...
 */
@Entity
@Table(name="TB_DINING")
@SequenceGenerator(
        name = "SQ_DINING_GENERATOR"
        , sequenceName = "SQ_DINING"
        , initialValue = 1
        , allocationSize = 1
)
@Getter
...
@SQLDelete(sql = "UPDATE TB_DINING SET DELETE_YN = 'Y', DELETE_TIME=TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS') WHERE DNO = ?")

public class Dining extends BaseTimeEntity{
    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE,
    generator = "SQ_DINING_GENERATOR")
    private Integer dno;
